...
/*   Updated: 2016/05/19 20:02:54 by tiboitel         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void		wolf3d_core(t_wolf3d *wolf)
{
	...
	quit = 1;
	texture = SDL_CreateTexture(wolf->renderer, SDL_PIXELFORMAT_ARGB8888, SDL_TEXTUREACCESS_STREAMING, WINDW_H, WINDW_W);
	...
	wolf->raycaster.planey = 5.00;
	while (quit)
	{
		current_time = SDL_GetTicks();
		SDL_UpdateTexture(texture, NULL, pixels, WINDW_H * sizeof(unsigned int));
	...
	// Calculate x-coordinate on the camera plane.
		...
		while (x < WINDW_W)
		{
			...
			while (wolf->raycaster.hit == 0)
			{
				...
				if ((wolf->map->map[wolf->raycaster.mapx][wolf->raycaster.mapy] - 48) > 0)
			...
			{
				pixels[i * WINDW_H + x] = 0xFF;
				i++;
			}
			x++;
		}
		SDL_RenderClear(wolf->renderer);
		SDL_RenderCopy(wolf->renderer, texture, NULL, NULL);
